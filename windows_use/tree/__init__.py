...
class Tree:
    ...
    def get_appwise_nodes(self,node:Control) -> tuple[list[TreeElementNode],list[TextElementNode],list[ScrollElementNode]]:
        ...
        if visible_apps:
            foreground_app_name = next(iter(visible_apps))
            foreground_app = visible_apps[foreground_app_name]
    ...
    def get_nodes(self, node: Control) -> tuple[list[TreeElementNode],list[TextElementNode],list[ScrollElementNode]]:
        ...
        def is_element_interactive(node:Control):
            try:
                # Explicitly handle ImageControl first
                if node.ControlTypeName == 'ImageControl':
                    # An ImageControl is only interactive if it's visible, enabled, AND has a name.
                    # A nameless one is considered decorative.
                    return is_element_visible(node) and is_element_enabled(node) and bool(node.Name.strip())

                if node.ControlTypeName in INTERACTIVE_CONTROL_TYPE_NAMES:
                    # For all other interactive types, the original logic is fine.
                    # `is_element_image` is no longer needed here.
                    if is_element_visible(node) and is_element_enabled(node):
                        return True
